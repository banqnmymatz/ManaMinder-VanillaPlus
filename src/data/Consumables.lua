local consumables = {
  ["MINOR_MANA_POTION"] = {
    name = "Minor Mana Potion",
    key = "MINOR_MANA_POTION",
    group = "POTION",
    itemId = 2455,
    iconTexture = "Interface\\Icons\\INV_Potion_70",
    requiredDeficit = 181,
    type = "ITEM"
  },
  ["LESSER_MANA_POTION"] = {
    name = "Lesser Mana Potion",
    key = "LESSER_MANA_POTION",
    group = "POTION",
    itemId = 3385,
    iconTexture = "Interface\\Icons\\INV_Potion_71",
    requiredDeficit = 361,
    type = "ITEM"
  },
  ["MANA_POTION"] = {
    name = "Mana Potion",
    key = "MANA_POTION",
    group = "POTION",
    itemId = 3827,
    iconTexture = "Interface\\Icons\\INV_Potion_72",
    requiredDeficit = 586,
    type = "ITEM"
  },
  ["GREATER_MANA_POTION"] = {
    name = "Greater Mana Potion",
    key = "GREATER_MANA_POTION",
    group = "POTION",
    itemId = 6149,
    iconTexture = "Interface\\Icons\\INV_Potion_73",
    requiredDeficit = 901,
    type = "ITEM"
  },
  ["COMBAT_MANA_POTION"] = {
    name = "Combat Mana Potion",
    key = "COMBAT_MANA_POTION",
    group = "POTION",
    itemId = 18841,
    iconTexture = "Interface\\Icons\\INV_Potion_81",
    requiredDeficit = 1501,
    type = "ITEM"
  },
  ["SUPERIOR_MANA_POTION"] = {
    name = "Superior Mana Potion",
    key = "SUPERIOR_MANA_POTION",
    group = "POTION",
    itemId = 13443,
    iconTexture = "Interface\\Icons\\INV_Potion_74",
    requiredDeficit = 1501,
    type = "ITEM"
  },
  ["MAJOR_MANA_POTION"] = {
    name = "Major Mana Potion",
    key = "MAJOR_MANA_POTION",
    group = "POTION",
    itemId = 13444,
    iconTexture = "Interface\\Icons\\INV_Potion_76",
    requiredDeficit = 2250,
    type = "ITEM"
  },
  ["MAJOR_REJUVENATION_POTION"] = {
    name = "Major Rejuvenation Potion",
    key = "MAJOR_REJUVENATION_POTION",
    itemId = 18253,
    iconTexture = "Interface\\Icons\\INV_Potion_47",
    requiredDeficit = 2061,
    type = "ITEM"
  },
  ["DEMONIC_RUNE"] = {
    name = "Demonic Rune",
    key = "DEMONIC_RUNE",
    group = "RUNE",
    itemId = 12662,
    iconTexture = "Interface\\Icons\\INV_Misc_Rune_04",
    requiredDeficit = 1500,
    type = "ITEM"
  },
  ["DARK_RUNE"] = {
    name = "Dark Rune",
    key = "DARK_RUNE",
    group = "RUNE",
    itemId = 20520,
    iconTexture = "Interface\\Icons\\Spell_Shadow_SealOfKings",
    requiredDeficit = 1500,
    type = "ITEM"
  },
  ["LILY_ROOT"] = {
    name = "Lily Root",
    key = "LILY_ROOT",
    group = "RUNE",
    itemId = 14894,
    iconTexture = "Interface\\Icons\\INV_Misc_Herb_02",
    requiredDeficit = 676,
    type = "ITEM"
},
  ["NIGHT_DRAGONS_BREATH"] = {
    name = "Night Dragon's Breath",
    key = "NIGHT_DRAGONS_BREATH",
    group = "RUNE",
    itemId = 11952,
    iconTexture = "Interface\\Icons\\INV_Misc_Food_45",
    requiredDeficit = 1100,
    type = "ITEM"
},
  ["RAZORLASH_ROOT"] = {
    name = "Razorlash Root",
    key = "RAZORLASH_ROOT",
    group = "RUNE",
    itemId = 17747,
    iconTexture = "Interface\\Icons\\INV_Misc_Herb_07",
    requiredDeficit = 2701,
    type = "ITEM"
  },
  ["MANA_AGATE"] = {
    name = "Mana Agate",
    key = "MANA_AGATE",
    group = "GEM",
    itemId = 5514,
    iconTexture = "Interface\\Icons\\INV_Misc_Gem_Emerald_01",
    requiredDeficit = 426,
    type = "ITEM",
    class = "MAGE"
  },
  ["MANA_JADE"] = {
    name = "Mana Jade",
    key = "MANA_JADE",
    group = "GEM",
    itemId = 5513,
    iconTexture = "Interface\\Icons\\INV_Misc_Gem_Emerald_02",
    requiredDeficit = 651,
    type = "ITEM",
    class = "MAGE"
  },
  ["MANA_CITRINE"] = {
    name = "Mana Citrine",
    key = "MANA_CITRINE",
    group = "GEM",
    itemId = 8007,
    iconTexture = "Interface\\Icons\\INV_Misc_Gem_Opal_01",
    requiredDeficit = 926,
    type = "ITEM",
    class = "MAGE"
  },
  ["MANA_RUBY"] = {
    name = "Mana Ruby",
    key = "MANA_RUBY",
    group = "GEM",
    itemId = 8008,
    iconTexture = "Interface\\Icons\\INV_Misc_Gem_Ruby_01",
    requiredDeficit = 1201,
    type = "ITEM",
    class = "MAGE"
  },
  ["HOLY_SPRING_WATER"] = {
    name = "Holy Spring Water",
    key = "HOLY_SPRING_WATER",
    group = "SPRING",
    itemId = 737,
    iconTexture = "Interface\\Icons\\INV_Potion_01",
    requiredDeficit = 3801,
    type = "ITEM",
  },
  ["SCROLL_OF_MANA_CHANNELING"] = {
    name = "Scroll of Mana Channeling",
    key = "SCROLL_OF_MANA_CHANNELING",
    group = "SCROLL",
    itemId = 26089,
    iconTexture = "Interface\\Icons\\INV_Scroll_04purple",
    requiredDeficit = 100,
    type = "ITEM",
  }
}

ManaMinder.consumables = consumables
